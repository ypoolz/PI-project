...
def menu_clientes():
    ...
    def deletar_clientes():
        janela = tk.Tk()
        janela.title('Deletar Cliente ')

        def deletar_idclientes():
            try:
                conexao = sqlite3.connect('DBdoAPP.db')
        ...
        label_id = tk.Label(janela, text='Id do Usuário:')
        label_id.grid(row=0, column=0, padx=10, pady=10)
        
        entry_id = tk.Entry(janela, width=30)
        entry_id.grid(row=0, column=1, padx=10, pady=10)

        botao_id = tk.Button(janela, text='Deletar Cliente', command=deletar_idclientes)
        botao_id.grid(row=1, column=0, padx=10, pady=10, columnspan=2, ipadx=80)
